import { useEffect, useState } from 'react';
import { api, formatPesos } from '../lib/api';
import { useAuthGuard } from '../lib/useAuthGuard';
import type { Producto } from '../lib/types';

const emptyForm = { nombre: '', precio: '', stock: '' };

export default function ProductosApp() {
	const ready = useAuthGuard();
	const [productos, setProductos] = useState<Producto[]>([]);
	const [loading, setLoading] = useState(true);
	const [editingId, setEditingId] = useState<number | null>(null);
	const [form, setForm] = useState(emptyForm);
	const [error, setError] = useState<string | null>(null);
	const [saving, setSaving] = useState(false);

	async function loadProductos() {
		setLoading(true);
		try {
			setProductos(await api.productos.list());
		} catch {
			setError('No se pudieron cargar los productos');
		} finally {
			setLoading(false);
		}
	}

	useEffect(() => {
		if (ready) loadProductos();
	}, [ready]);

	function startEdit(producto: Producto) {
		setEditingId(producto.id);
		setForm({
			nombre: producto.nombre,
			precio: (producto.precio_centavos / 100).toString(),
			stock: producto.stock !== null ? producto.stock.toString() : '',
		});
	}

	function cancelEdit() {
		setEditingId(null);
		setForm(emptyForm);
	}

	async function handleSubmit(e: React.FormEvent) {
		e.preventDefault();
		setError(null);
		setSaving(true);

		const input = {
			nombre: form.nombre,
			precio_centavos: Math.round(parseFloat(form.precio) * 100),
			stock: form.stock.trim() === '' ? null : parseInt(form.stock, 10),
		};

		try {
			if (editingId) {
				await api.productos.update(editingId, input);
			} else {
				await api.productos.create(input);
			}
			cancelEdit();
			await loadProductos();
		} catch {
			setError('No se pudo guardar el producto');
		} finally {
			setSaving(false);
		}
	}

	async function handleDelete(id: number) {
		if (!confirm('Eliminar este producto?')) return;
		try {
			await api.productos.remove(id);
			await loadProductos();
		} catch {
			setError('No se pudo eliminar el producto');
		}
	}

	if (!ready) return null;

	return (
		<div className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
			<h1 className="text-2xl font-bold text-slate-800">Productos</h1>

			<form
				onSubmit={handleSubmit}
				className="bg-white rounded-lg shadow p-4 flex flex-col gap-3"
			>
				<h2 className="font-semibold text-slate-700">
					{editingId ? 'Editar producto' : 'Nuevo producto'}
				</h2>

				<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
					<input
						placeholder="Nombre"
						value={form.nombre}
						onChange={(e) => setForm({ ...form, nombre: e.target.value })}
						className="border border-slate-300 rounded px-3 py-2 sm:col-span-1"
						required
					/>
					<input
						placeholder="Precio"
						type="number"
						step="0.01"
						min="0"
						value={form.precio}
						onChange={(e) => setForm({ ...form, precio: e.target.value })}
						className="border border-slate-300 rounded px-3 py-2"
						required
					/>
					<input
						placeholder="Stock (opcional)"
						type="number"
						min="0"
						value={form.stock}
						onChange={(e) => setForm({ ...form, stock: e.target.value })}
						className="border border-slate-300 rounded px-3 py-2"
					/>
				</div>

				{error && <p className="text-sm text-red-600">{error}</p>}

				<div className="flex gap-2">
					<button
						type="submit"
						disabled={saving}
						className="bg-slate-800 text-white rounded px-4 py-2 font-medium hover:bg-slate-700 disabled:opacity-50"
					>
						{saving ? 'Guardando...' : editingId ? 'Guardar cambios' : 'Crear producto'}
					</button>
					{editingId && (
						<button
							type="button"
							onClick={cancelEdit}
							className="text-slate-600 px-4 py-2"
						>
							Cancelar
						</button>
					)}
				</div>
			</form>

			<div className="bg-white rounded-lg shadow overflow-x-auto">
				{loading ? (
					<p className="p-4 text-slate-500">Cargando...</p>
				) : productos.length === 0 ? (
					<p className="p-4 text-slate-500">Todavia no hay productos cargados.</p>
				) : (
					<table className="w-full text-sm">
						<thead>
							<tr className="bg-slate-100 text-left text-slate-600">
								<th className="p-3">Nombre</th>
								<th className="p-3">Precio</th>
								<th className="p-3">Stock</th>
								<th className="p-3"></th>
							</tr>
						</thead>
						<tbody>
							{productos.map((producto) => (
								<tr key={producto.id} className="border-t border-slate-100">
									<td className="p-3 font-medium text-slate-800">
										{producto.nombre}
									</td>
									<td className="p-3 text-slate-600">
										{formatPesos(producto.precio_centavos)}
									</td>
									<td className="p-3 text-slate-600">
										{producto.stock ?? '-'}
									</td>
									<td className="p-3 text-right whitespace-nowrap">
										<button
											onClick={() => startEdit(producto)}
											className="text-slate-600 hover:text-slate-900 mr-3"
										>
											Editar
										</button>
										<button
											onClick={() => handleDelete(producto.id)}
											className="text-red-600 hover:text-red-800"
										>
											Eliminar
										</button>
									</td>
								</tr>
							))}
						</tbody>
					</table>
				)}
			</div>
		</div>
	);
}
